...
with open('themes', 'r') as f:
    # a file for default themed dictionaries
...
def pay(msg):
    if 'paid' not in msg.text:
        bot.send_message(
            msg.chat.id, 'You can pay with qiwi. Send me your number, pay to number +71234567890 and send /pay paid')
        data[str(msg.chat.id)]['status'] = 'enter qiwi number'
    else:
        data[str(msg.chat.id)]['status'] = ''
        txs = list(filter(lambda x: x.type == 'IN',
                          w.history()['transactions']))
        for tnx in txs:
...
def dicts(msg):
    ...
    for i, dct in enumerate(data[str(msg.chat.id)]['dicts']):
        keyboard.add(types.InlineKeyboardButton(
            text=dct['name'], callback_data='dict:' + str(i)))
    keyboard.add(types.InlineKeyboardButton(
        text="New dict", callback_data="new_dict"))
...
def theme_dicts(msg):
    ...
    for i in themes:
        keyboard.add(types.InlineKeyboardButton(
            text=i, callback_data='global_dict:' + str(i)))
    bot.send_message(msg.chat.id, 'Thematic dictionaries:',
...
def callback_inline(call):
    ...
    if call.data == 'new_dict':
        ...
        keyboard = types.InlineKeyboardMarkup()
        keyboard.add(types.InlineKeyboardButton(
            text='train', callback_data='train:_' + call.data.split(':')[1]))
        bot.send_message(chat, '<b>' + call.data.split(':')[1] + '</b>\n' + str(len(
            themes[call.data.split(':')[1]]))+' words', reply_markup=keyboard, parse_mode='html')
    elif call.data.split(':')[0] == 'add':
        data[str(chat)]['dicts'][int(call.data.split(':')[1])
                                 ]['words'].append(call.data.split(':')[2])
        bot.send_message(chat, 'word added to dict')
    elif call.data.split(':')[0] == 'dict':
        keyboard = types.InlineKeyboardMarkup()
        keyboard.add(types.InlineKeyboardButton(
            text='train', callback_data='train:' + call.data.split(':')[1]))
        bot.send_message(chat, '<b>' + data[str(chat)]['dicts'][int(call.data.split(':')[1])]['name'] + '</b>\n' + str(len(
            data[str(chat)]['dicts'][int(call.data.split(':')[1])]['words']))+' words', reply_markup=keyboard, parse_mode='html')
        ...
        if call.data.split(':')[1].startswith('_'):
            ...
            r = random.choice(data[str(chat)]['dicts']
                              [int(call.data.split(':')[1])]['words'])
        ...
        if rnd == 1:
            ...
            bot.send_message(
                chat, '<b>' + revdic[r.split('\n')[0]] + '</b>', parse_mode='html')
        else:
            say(r.split('\n')[0])
            with open('output.mp3', 'rb') as f:
                bot.send_voice(chat, f)
        data[str(chat)]['status'] = 'train:' + \
            call.data.split(':')[1] + ':' + r.split('\n')[0]
...
def ans(msg):
    ...
    if data[str(chat)]['status'] == 'new_dict':
        ...
        if data[str(chat)]['status'].split(':')[1].startswith('_'):
            r = random.choice(
                themes[data[str(chat)]['status'].split(':')[1][1:]])
        else:
            r = random.choice(data[str(chat)]['dicts'][int(
                data[str(chat)]['status'].split(':')[1])]['words'])
        rnd = random.randint(1, 3)
        if rnd == 1:
            bot.send_message(chat, '<b>' + r.split('\n')
                             [0] + '</b>\n' + ''.join(r.split('\n')[1:]), parse_mode='html')
            ...
            with open('output.mp3', 'rb') as f:
                bot.send_voice(chat, f)
        data[str(chat)]['status'] = 'train:' + data[str(chat)
                                                    ]['status'].split(':')[1] + ':' + r.split('\n')[0]
    else:
        keyboard = types.InlineKeyboardMarkup()
        [keyboard.add(types.InlineKeyboardButton(text='add to '+dct['name'], callback_data='add:' +
                                                 str(i) + ':' + eng(msg.text))) for i, dct in enumerate(data[str(msg.chat.id)]['dicts'])]
